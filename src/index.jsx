...
function App() {
  ...
  const [toastStatus, setToastStatus] = useState("neutral");
  ...
  const toastPositions = [
    { icon: <CgArrowTopLeft />, position: "top-left" },
    { icon: <CgArrowUp />, position: "top-center" },
    { icon: <CgArrowTopRight />, position: "top-right" },
    { icon: <CgArrowBottomLeft />, position: "bottom-left" },
    { icon: <CgArrowDown />, position: "bottom-center" },
    { icon: <CgArrowBottomRight />, position: "bottom-right" },
  ];

  const handleToast = ({ title, msg, status, position, time }) => {
    status = toastStatus;
    time = 2000;
    const toastFunction = toast[status] || toast.neutral;
    toastFunction({ title, msg, time, position, status });
  };
      ...
      <div className="parent-container">
        ...
        <div>
          <Select
            ...
            initialSelected={{
              value: toastStatus,
              label: toastStatus.charAt(0).toUpperCase() + toastStatus.slice(1),
            }}
            onChange={(t) => setToastStatus(t)}
        ...
        <div className="grid">
          {toastPositions.map((p) => (
            <Button onClick={() => handleToast({ position: p.position })}>
              {p.icon}
            </Button>
          ))}
          <ToastContainer />
